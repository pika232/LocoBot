...
def getGoals(): # 使用正弦波函数模拟goal 的位置变化
    fs = 10.  # sample rate
...
class RobotControl():
    ...
    def __init__(self):
        ...
        self.cmd = Twist()
        self.ctrl_c = False
        self.rate = rospy.Rate(10) # Rate that the node loops
        self.rate.sleep()
    ...
    def moveGoal_pid(self):
        ...
        while (self.euclidean_distance() > self.distance_tolerance):
            ...
            linear_velocity = self.ANGULAR_VELOCITY * e + self.distance_tolerance * e_sum + self.LINEAR_VELOCITY * dedt
            ...
            self.cmd.angular.z = self.ANGULAR_VELOCITY #??? constant?
