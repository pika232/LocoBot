...
def getGoals(): # Use sine wave to simulate how the goal moves
    ...
    # !!!!! 
    # In other case, the coordinate system of goal must be check again whether is the same as odometry or not (?)
...
class RobotControl():
    ...
    def __init__(self):
        ...
        self.cmd = Twist()
        self.ctrl_c = False
        self.rate = rospy.Rate(10) # Loop frequency of the node (publishing and reading)
        self.rate.sleep()

        # TO-DO Modify the value of these 3 constants after testing on robot or reading from somewhere.. 
        self.LINEAR_VELOCITY = 0.2
        self.ANGULAR_VELOCITY = 0.5
        self.distance_tolerance =self.LINEAR_VELOCITY/2
    ...
    def goal_orientation(self):
        ...
        """
        theta_r = self.pose[2]
        theta_g = math.atan2(self.goal[1] - self.pose[1], self.goal[0] - self.pose[0]) # (y_g -y_r, x_g - x_r)
        if abs(theta_r - theta_g) <math.pi :
            return theta_r - theta_g
        else: 
            return 2* math.pi - (theta_r - theta_g)
    ...
    def moveGoal_pid(self):
        ...
        dt = 1./self.rate #TIme between 2 updates
        e_sum = 0.
        w_sum = 0.
        ...
        while (self.euclidean_distance() > self.distance_tolerance and not self.ctrl_c):
            ...
            linear_velocity = min(linear_velocity, self.LINEAR_VELOCITY)  
            ...
            w = self.goal_orientation() # error of angle
            dwdt = (w-w_prev)/dt
            w_sum += w * dt
            w_prev = w
